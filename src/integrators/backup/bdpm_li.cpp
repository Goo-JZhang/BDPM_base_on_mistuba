...
public:
    ...
    */
    ...
    void process(const WorkUnit *workUnit, WorkResult *workResult, const bool&stop)
    {
        ...
        m_sampler->request2DArray(16);
        m_sampler->request2DArray(32);
        ...
        Float diffScaleFactor = 1.0f/std::sqrt((Float) spp);
        ...
        for(size_t i = 0; i<m_hilberCurve.getPointCount(); ++i)
        {
            ...
            for(size_t j = 0; j< spp ; j++)
            {
                ...
                RayDifferential ray;
                Spectrum throughput  = m_sensor->sampleRayDifferential(ray, Point2(offset), needsApertureSample? m_sampler->next2D():Point2(0.5f), stime);

                ray.scaleDifferential(diffScaleFactor);

                throughput *=Li(ray, rRec);

                result->putSample(samplePos, throughput);
    ...
    Spectrum Li(const RayDifferential &ray, RadianceQueryRecord &rRec) const {
        ...
        bool exhaustiveSpecular = !cacheQuery;

        if (isDiffuse && (dot(its.shFrame.n, ray.d) < 0 || (bsdf->getType() & BSDF::EBackSide))) {
            /* 1. Diffuse indirect */
            int maxDepth = m_config.maxDepth == -1 ? INT_MAX : (m_config.maxDepth-rRec.depth);
            if (rRec.type & RadianceQueryRecord::ERadianceNoEmission && m_photonMap.get())
                LiSurf += m_photonMap->estimateIrradiance(its.p,
                    its.shFrame.n, m_config.maxRadius, maxDepth,
                    m_config.maxPhoton) * bsdf->getDiffuseReflectance(its) * INV_PI;
            //std::cout<<LiSurf.toString()<<std::endl;
        }
        ...
        if (hasSpecular && exhaustiveSpecular
            ...
            for (int i=0; i<compCount; i++) {
                ...
                LiSurf += bsdfVal * this->Li(bsdfRay, rRec2);
            }
        }
        ...
        int m_directSamples = 16;
        int m_glossySamples = 32;
        int numEmitterSamples = m_directSamples, numBSDFSamples;
        int m_invEmitterSamples = 1.0f / m_directSamples;
        int m_invGlossySamples = 1.0f / m_glossySamples;
        ...
        if (bsdfSampleDirect || bsdfSampleIndirect) {
            ...
            for (int i=0; i<numBSDFSamples; ++i) {
                ...
                if (bsdfSampleIndirect) {
                    ...
                    LiSurf += bsdfVal * this->Li(bsdfRay, rRec2) * weightBSDF;
                }
            }
        }
